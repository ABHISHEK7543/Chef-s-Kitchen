...
const Cards = () => {
  ...
  return (
    <div className="mt-10">
      <h1 className="abhi font-bold text-3xl ">Blog Post</h1>
      <button className="py-3 px-6 rounded-full bg-black text-white mt-4">
        All category
      </button>
      ...
      <div className="flex flex-wrap justify-between mt-8">
        {data.map((elem, idx) => {
          return (
            <div
              key={idx}
              className="card h-80 w-100 bg-amber-100 mr-4 mb-4  rounded-2xl"
            >
              <img
                className="w-full h-[70%] object-fill rounded-2xl"
                src={elem.img}
                alt=""
              />
              <div className="px-4">
      ...
      <h1 id="abhi" className="font-bold mb-10">
        Top chefName{" "}
      </h1>
      <div className="flex justify-between flex-wrap gap-5 ">
        {chefimg.map((value, id) => {
          return (
            <div
              key={id}
              className="card h-100 w-80 bg-amber-200 rounded-2xl relative overflow-hidden  "
            >
              <img className="h-full w-full object-cover" src={value.pic} />

              <div
                id="bottom"
                className="h-24 w-60 m-8 bg-white rounded-2xl absolute bottom-0 text-center "
              >
                <h1>{value.name}</h1>
                <button className="py-2 px-4 rounded-full bg-black text-white mt-2">
                  Stream Now
                </button>
              </div>
            </div>
      ...
      <div className="w-full h-120 mt-10 bg-[url(https://i.pinimg.com/736x/14/01/25/1401256df4d81b17b6d34fb107fb2adb.jpg)] bg-cover flex items-center justify-center">
        <div>
          <h1 className="text-4vw text-center">Fresh Insiperation via email</h1>
          <div>
            <input
              className="emailinput outline-4 rounded-2xl py-3 px-8 mr-2"
              type="text"
              placeholder="Your email address"
            />

            <button className="sub py-4 px-4 rounded-full bg-black text-white mt-2">
              Subscribe Now
            </button>
          </div>
        </div>
      ...
      <footer
        id="force"
        className="flex-wrap flex items-center justify-center text-gray-500 mt-10"
      >
